...
**Tests**: Behavior-changing work MUST include test tasks. Tests may be omitted only
for documentation, configuration, or other explicitly non-behavioral work, and the
reason MUST be recorded in the plan.
...
- [ ] T004 Load and record applicable Markdown instruction files
- [ ] T005 [P] Configure static analysis for duplication, long functions/methods,
  God-class indicators, secret scanning and project-specific thresholds
...
instruction chain, test policy and quality-gate configuration are foundational.
...
- [ ] T012 Configure `framework check` to run locally and in CI with actionable
  file/line/metric output
...
- [ ] T021 [US1] Run static analysis and resolve or justify new quality violations
...
- [ ] TXXX Run `framework check`, review the diff and record approved exceptions
- [ ] TXXX Run `framework security scan` and confirm no secret is present in the
  workspace, staged diff or remote-bound history
- [ ] TXXX [OPTIONAL] Run `framework learn review` and review proposed lessons,
  rework signals and worktree boundary diagnostics without authorizing parallel work
- [ ] TXXX [OPTIONAL] Run `framework learn quiz`, inspect questions linked to changed
  symbols/rules and mark obsolete questions for review
