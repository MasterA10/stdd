...
def build_parser() -> argparse.ArgumentParser:
    ...
    handoff_send = handoff_sub.add_parser("send"); handoff_send.add_argument("package"); handoff_send.add_argument("--target", required=True, choices=("codex", "claude", "cloud", "antigravity", "generic")); handoff_send.add_argument("--dry-run", action="store_true"); handoff_send.add_argument("path", nargs="?", default="."); _common(handoff_send)
    hooks = learn_sub.add_parser("hooks"); hooks_sub = hooks.add_subparsers(dest="hooks_command", required=True)
    hooks_install = hooks_sub.add_parser("install"); hooks_install.add_argument("--host", dest="hosts", action="append", choices=("codex", "claude", "cloud", "antigravity", "generic")); hooks_install.add_argument("path", nargs="?", default="."); _common(hooks_install)
    hooks_event = hooks_sub.add_parser("event"); hooks_event.add_argument("--host", default="generic", choices=("codex", "claude", "cloud", "antigravity", "generic")); hooks_event.add_argument("--event", required=True); hooks_event.add_argument("--session-id"); hooks_event.add_argument("--facts", help="JSON facts from the host hook; redacted before persistence"); hooks_event.add_argument("path", nargs="?", default="."); _common(hooks_event)
    learn_quiz = learn_sub.add_parser("quiz")
    learn_quiz_sub = learn_quiz.add_subparsers(dest="learn_quiz_command", required=True)
    learn_gen = learn_quiz_sub.add_parser("generate"); learn_gen.add_argument("--agent", "--provider", dest="learn_provider", choices=("local", "codex", "claude", "cloud", "antigravity", "generic"), default="local"); learn_gen.add_argument("--scope", dest="learn_scope", default="project"); learn_gen.add_argument("path", nargs="?", default="."); _common(learn_gen)
    ...
    gen = quiz_sub.add_parser("generate"); gen.add_argument("--agent", "--provider", dest="agent", choices=("local", "codex", "claude", "cloud", "antigravity", "generic"), default="local"); gen.add_argument("--scope", default="project"); gen.add_argument("path", nargs="?", default="."); _common(gen)
