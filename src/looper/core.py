...
- Quando for necessário usar subagentes, use o Herdr (modo nativo de agentes) em uma pane visível e mantenha o modo direto headless como padrão operacional; o modo interativo/TUI só deve ser usado quando o usuário pedir acompanhamento visual interativo ou intervenção manual. Pergunte ao usuário antes de iniciar, salvo quando ele já tiver solicitado subagentes claramente; não ofereça `terminal` como alternativa para subagentes.
