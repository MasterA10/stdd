...
- Quando for necessário usar subagentes, use sempre o Herdr com a TUI e os agentes/hooks nativos em uma pane visível. O agente principal deve acompanhar somente estados de ciclo de vida, nunca ler scrollback, `herdr agent read`, `pane read` ou output intermediário; instrua o subagente a gravar apenas o resultado final em Markdown/artefato e leia somente esse arquivo. Headless fica como exceção explícita solicitada pelo usuário ou pelo contrato. Pergunte ao usuário antes de iniciar, salvo quando ele já tiver solicitado subagentes claramente; não ofereça `terminal` como alternativa para subagentes.
