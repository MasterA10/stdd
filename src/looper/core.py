...
- Quando for necessário usar subagentes, use sempre o Herdr com a TUI e os agentes/hooks nativos em uma pane visível. O agente principal deve acompanhar primeiro os estados de ciclo de vida e pode fazer, somente sob demanda, uma leitura curta de até 20 linhas (`herdr agent read <agent> --source visible --lines 20`) após esperar um período proporcional à complexidade da tarefa. Se a leitura confirmar que o subagente está trabalhando e avançando, não faça novas leituras automáticas: deixe-o trabalhar até `idle`, `done` ou `blocked`; só leia novamente diante de bloqueio, suspeita concreta de travamento, mudança relevante de estado ou pedido do usuário. Não use polling, timeouts curtos, `pane read`, `terminal session observe` ou scrollback para transportar contexto. Entregue ao subagente um único prompt inicial completo, incluindo escopo, critérios, testes e artefato final; não envie comandos rotineiros de continuação ou finalização. Leia somente o artefato final para coletar o resultado completo. Headless fica como exceção explícita solicitada pelo usuário ou pelo contrato. Pergunte ao usuário antes de iniciar, salvo quando ele já tiver solicitado subagentes claramente; não ofereça `terminal` como alternativa para subagentes.
...
- As descrições dos nós de todos os Draws devem ter entre 100 e 250 caracteres, contando o texto após remover espaços nas extremidades. A análise estática emite o warning `draw.description_length` quando o limite é violado; descrições longas devem ser condensadas, desmembradas em subfluxo ou complementadas por perguntas, sem texto repetitivo ou nós decorativos.
...
- O Design System deve declarar explicitamente a família de fontes, fontes alternativas, pesos disponíveis, escala tipográfica e line-heights. Defina tokens globais semânticos para `font-family`, tamanho, peso, altura de linha, tracking e cor; componentes e telas devem herdar esses tokens e não podem escolher `font-size`, `font-weight`, `line-height` ou cores ad hoc por trecho.
- Mantenha uma escala tipográfica centralizada e nomeada (por exemplo `display`, `h1`, `h2`, `h3`, `h4`, `body`, `body-sm`, `label` e `code`), com valores explícitos para desktop e mobile em `rem` e breakpoints documentados. Use os mesmos papéis em todas as telas; só crie um novo token quando houver uma necessidade visual recorrente e documentada.
- Cores também devem ser tokens semânticos herdáveis, separados entre papéis (fundo, superfície, texto primário/secundário, borda, accent e estados) e valores. Não replique hex/rgb diretamente em componentes. Garanta que o tema global seja aplicado na raiz e que exceções locais sejam raras, justificadas e registradas no `.looper/design.html`.
- Modularidade de frontend: ao criar estilos, scripts e marcação, prefira módulos reutilizáveis e arquivos pequenos quando a stack permitir. Em CSS externo, quebre a estilização por responsabilidade, componente ou tela relacionada; não concentre toda a página em uma folha grande. Em Tailwind ou stacks equivalentes, respeite a organização idiomática da própria stack, mesmo que isso resulte em poucos arquivos CSS externos. Para JavaScript e HTML, extraia componentes, módulos e templates reutilizáveis quando houver suporte nativo; se a stack não oferecer esse mecanismo, adapte a estrutura sem forçar uma divisão artificial.
...
- Playwright como validador rigoroso ("carrasco"): a suíte de testes de interface opera sob regras estritas de fidelidade à experiência do usuário final. Proibição de bypass de dados: fica terminantemente vetado ao teste Playwright injetar dados diretamente no banco de dados ou acionar requisições HTTP artificiais para simular ações de tela. Exceção única para seeds de acesso: permite-se apenas a injeção inicial de um registro de usuário (seed) para autenticação caso o sistema não contemple um fluxo de auto-cadastro na interface. Regra de cobertura orgânica: se o teste não consegue avançar porque falta uma funcionalidade de CRUD ou um botão na interface, o agente deve implementar o recurso na UI, em vez de burlar o teste via script interno; toda e qualquer interação deve acontecer via cliques, digitação e navegação real.
...
- Quando qualquer trabalho tocar backend, logging, correção de bugs ou subagentes, se algo for reaproveitável, modularize, centralize e reutilize. Procure uma implementação compartilhada antes de criar lógica nova e evite duplicação.
- Trate os Draws como documentação viva oficial: mudanças de comportamento, fluxo ou decisão devem atualizar o Draw correspondente, suas conexões e símbolos antes da implementação. Quando a descrição do nó continuar correta e a mudança apenas acrescentar contexto, detalhe ou explicação, não edite a descrição: registre uma nova pergunta e uma resposta no nó mais relacionado. Edite a descrição somente quando ela estiver incorreta ou quando o comportamento, fluxo ou decisão documentada mudar.
---
{{convention_catalog}}
---
- Qualquer alteração de comportamento feita diretamente pelo agente ou por um comando de programação deve primeiro atualizar os Draws correspondentes e depois atender ao pedido do usuário, para que a documentação reflita o comportamento novo. Se a alteração não mudar o comportamento e acrescentar somente um detalhe específico, não edite uma descrição que já esteja correta: crie uma nova pergunta e uma resposta no Draw correto, respeitando a hierarquia L2/L3. Se a alteração mudar um fluxo ou o comportamento documentado, atualize a descrição e também as conexões dos nós quando necessário para representar o novo caminho. Os Draws são a documentação oficial do comportamento do sistema e devem permanecer atualizados.
