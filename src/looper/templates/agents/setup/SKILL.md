...
O setup também constrói e revisa o `.gitignore` da raiz. Deve preservar regras existentes e manter `.env`, `.env.*`, `*.pyc`, `__pycache__/`, `.cache/`, `**/.cache/`, `*.cache`, `.coverage`, `coverage/`, ambientes virtuais e caches de ferramentas. O `looper init` ignora o conteúdo de `.agents` por padrão e reabre somente `.agents/skills/` para skills extras, `.agents/conventions/` e seus arquivos; as skills oficiais continuam ignoradas por entradas explícitas. `.looper/runs/` e seus artefatos de evidência permanecem versionáveis. A sincronização é idempotente e a exceção `!.env.example` é permitida. Não criar arquivos de credencial nem copiar valores de `.env` para relatórios.
