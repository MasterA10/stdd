...
Durante `looper init`, o framework mantém um `.gitignore` na raiz do projeto e adiciona regras idempotentes para `.env`, `.env.*`, `*.pyc`, `__pycache__/`, ambientes virtuais e `node_modules/`, preservando regras preexistentes. As skills oficiais são ignoradas por diretório explícito, enquanto skills extras em `.agents/skills/` e arquivos de `.agents/conventions/` continuam versionáveis. A exceção `!.env.example` permite versionar apenas o modelo sem credenciais. Não crie nem registre arquivos `.env` como evidência.
