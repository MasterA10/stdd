...
A rastreabilidade desta fase é obrigatória: associar o símbolo real pelo `stdd draw associate-reference` e preservar `code_refs`. O gate também considera `draw.level2_missing_code_ref`, `draw.level3_missing_code_ref`, `draw.level4_missing_code_ref` e `draw.empty_node_symbol`.
