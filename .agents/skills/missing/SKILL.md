...
1. Execute `looper backlog task`.
2. Se a resposta for `backlog-test-required`, execute `looper backlog test`, crie os testes do nó de nível 2 e de todos os seus subfluxos ou marque manualmente no viewer um fluxo de sistema já existente; a análise estática é opcional para essa marcação.
...
7. Se ainda não houver testes do escopo, atenda primeiro `backlog-test-required` com `looper backlog test`; nessa fase crie somente os testes. Depois retome `backlog task` para implementar a produção.
8. Execute os testes aplicáveis e os gates do Looper.
9. Execute `looper backlog complete <task-id>` usando exatamente o ID retornado.
10. Repita o ciclo.
...
Erros devem ser modelados como consequências condicionais (`se`/`ou`) e validações devem ocorrer antes do efeito correspondente. Não converta uma funcionalidade planejada em sequência: mantenha-a terminal no grupo `Não implementado`. Cada interação recebe uma única task e testes de integração devem provar API, persistência, validação e efeitos reais.
