---
name: draw-answer
description: Responde perguntas de um desenho marcadas explicitamente com @looper usando a codebase e seus símbolos; quando a resposta não puder ser comprovada, associa o arquivo e o símbolo relevante ao nó.
---
...
Esse comando é somente leitura e apresenta as perguntas pendentes agrupadas por desenho e nó, com o símbolo associado, arquivo, evidências e limitações em linguagem natural. O JSON de `looper draw questions` continua sendo a fonte operacional para a investigação.
...
2. Consultar a codebase e, quando disponível, os fatos em `.looper/facts/`: símbolos, `qualified_name`, arquivo, posição, dependências, chamadas, testes, contratos, RPCs, procedures, migrations e schemas.
...
- remover somente `@looper` do `question.prompt`, preservando o restante da pergunta e o histórico;
...
- manter `question.answer` sem resposta e conservar `@looper`;
...
Explique a conclusão diretamente em uma ou duas frases humanas. Não repita `@looper` e não esconda a conclusão em código ou metadados.
...
```bash
looper log "Responde perguntas endereçadas do Draw" --type implementacao
```
