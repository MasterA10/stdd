...
Cada nó pode declarar `negative_spec` como texto opcional. Use-o para registrar condições objetivas de não aceite: se a condição ocorrer, a task não pode ser declarada concluída. Não crie critérios de sucesso ou aceite; a revisão transversal dos nós L3 pertence à skill `$negative-spec`.
