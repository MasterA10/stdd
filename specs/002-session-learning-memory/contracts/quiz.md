...
```text
framework learn quiz generate [--agent local|codex|claude|cloud|antigravity] [--scope SCOPE]
...
```json
{
  ...
  "status": "current",
  "provenance": {"command": "codex", "job_id": "job-opaque", "version": "1"}
}
```

`quiz run` does not require an agent command and does not reveal the answer before
