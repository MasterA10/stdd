...
- As integrações de revisão são subprocessos locais: Codex usa `codex exec`, Claude usa `claude -p` e Antigravity usa `agy -p`. Os comandos, modelo, reasoning e prompt são editáveis na seção `review`; exigem que o CLI escolhido esteja instalado e autenticado no ambiente. O modo não interativo do Antigravity depende de uma autenticação prévia em uma sessão `agy` interativa.
- Contratos oficiais consultados: [Codex non-interactive mode](https://www.mintlify.com/openai/codex/concepts/non-interactive-mode), [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code/cli-usage) e [Antigravity CLI headless mode](https://antigravity.google/docs/cli/headless/). O Looper não chama APIs remotas diretamente.
