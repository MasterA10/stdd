...
- `$e2e-tester` é acionada como interação comum após implementação; lê o nó L3 do Draw, injeta entrada via `curl` (webhook preferencial) ou diretamente no handler, encadeia passos não automáticos e valida pelo log do app. O mock cobre somente o payload de entrada (trigger externo); comportamento, persistência e integrações sempre rodam no contexto real com credenciais do `.env` (no `.gitignore`); nunca commite credenciais.
