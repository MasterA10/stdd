...
- Use o `AGENTS.md` para visão geral, operação, rastreabilidade, limites de escopo e instalação; use `.agents/conventions/` para contratos, arquitetura e outras documentações técnicas específicas e reutilizáveis.
...
- O backlog entrega nós L4 como detalhamento técnico da camada backend, reentregando o pai L3 junto a cada grupo configurável de L4 até esgotar os filhos; `backlog.l4_group_size` (padrão `3`) pode ser alterado por `looper backlog config --l4-group-size N`.
...
- `.looper/config.yaml:instructions` é um objeto `{backend, frontend, change}` injetado pelo framework no loop correspondente (`backend` também é fallback para loops `test` e `bootstrap`); deve ser relido a cada entrega e nunca conter segredos; projetos legados com `instructions` como string continuam funcionando por retrocompatibilidade.
- O backend preserva os eventos de log em formato bruto e completo, sujeito apenas à redaction cirúrgica de segredos; a visualização processual inicial exibe timestamp, evento e status/erro (por exemplo, `200` ou `400`) e permite abrir o detalhe bruto sob demanda. O resumo é responsabilidade do frontend e não substitui o registro persistido.
- A seção `review` de `.looper/config.yaml` configura a revisão opcional após tasks concluídas. `review.interval_tasks` define depois de quantas tasks o agente é chamado; `review.execution_mode` usa somente `tmux`. A chamada usa Codex por padrão, mas o comando configurado também pode apontar para Claude ou Agy. A ausência de uma change significa aprovação; qualquer lacuna deve ser registrada no Draw como change para o loop de correções, sem transformar a revisão em task de implementação.
- `looper init` inicializa e migra os arquivos do projeto e, em terminal interativo, abre o DrawServe na interface web de configurações; a edição persistente de `.looper/config.yaml` acontece pela API local do DrawServe, enquanto `--no-web` mantém o uso automatizável sem servidor.
- A revisão usa Agy por padrão (`agy -p ... --dangerously-skip-permissions`); Codex CLI (`codex exec`) também pode ser selecionado nas configurações. Os comandos, agente, modelo, reasoning e prompt são editáveis na seção `review`; toda revisão ou correção executada por subagente usa exclusivamente `tmux`, e o CLI escolhido precisa estar instalado e autenticado no ambiente.
- Contratos oficiais consultados: [Codex non-interactive mode](https://www.mintlify.com/openai/codex/concepts/non-interactive-mode), [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code/cli-usage) e [Antigravity CLI headless mode](https://antigravity.google/docs/cli/headless/). O Agy headless usa `-p` e `--dangerously-skip-permissions`; essa flag aprova execução de comandos e escrita de arquivos, portanto o prompt de revisão deve limitar a ação à criação de changes no Draw. O Looper não chama APIs remotas diretamente.
...
- Toda mudança realizada no projeto deve ser refletida no Draw quando afetar um comportamento, fluxo, decisão ou detalhe já documentado. Atualize diretamente a informação no nó mais relacionado; quando a mudança acrescentar uma explicação ou detalhe sem alterar o comportamento existente, também pode registrar uma pergunta e uma resposta no Draw. A resposta deve ser minimamente explicativa, ainda que breve, para que o Draw permaneça como documentação atualizada do projeto.
...
- Ao consumir APIs, apps, SDKs ou provedores externos, registre o nome do serviço, endpoint/contrato, autenticação e pré-condições em `.agents/conventions/`; consulte a documentação oficial atual antes de implementar e não invente payloads.
