...
- `$test-application` usa `playwright-cli`/Playwright para fluxos de navegador e valida a persistência no banco quando o Draw exigir; integrações externas ficam determinísticas por padrão e testes live são opt-in com credenciais do `.env` (no `.gitignore`). Nunca commite credenciais.
...
- A seção `review` de `.looper/config.yaml` configura a revisão opcional após tasks concluídas. `review.interval_tasks` define depois de quantas tasks o agente é chamado; `review.execution_mode` aceita `terminal` (padrão) ou `tmux`. A chamada usa Codex por padrão, mas o comando configurado também pode apontar para Claude ou Agy. A ausência de uma change significa aprovação; qualquer lacuna deve ser registrada no Draw como change para o loop de correções, sem transformar a revisão em task de implementação.
...
- A revisão usa Agy por padrão (`agy -p ... --dangerously-skip-permissions`); Codex CLI (`codex exec`) também pode ser selecionado nas configurações. Os comandos, agente, modelo, reasoning, modo (`terminal`/`tmux`) e prompt são editáveis na seção `review`; o CLI escolhido precisa estar instalado e autenticado no ambiente.
