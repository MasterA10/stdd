...
export const CONFIG_TEXTS: Record<string, ConfigText> = {
  ...
  'review.interval_tasks': { label: 'Revisar a cada quantas tasks', description: 'Define depois de quantas tasks concluídas o agente será chamado.', hint: 'Use 1 para revisar cada task ou um número maior para revisar em lotes.', detail: 'A revisão não cria uma nova task de implementação. O agente é chamado via Herdr no modo nativo de agentes e, quando encontra um problema, registra uma change no Draw correspondente.' },
  'review.execution_mode': { label: 'Forma de chamar o agente', description: 'Escolha como o processo do agente será executado localmente.', hint: 'Herdr (modo nativo de agentes) é o padrão recomendado; terminal direto executa como processo local.' },
...
export const OPTION_TEXTS: Record<string, Record<string, OptionText>> = {
  ...
  'review.execution_mode': { herdr: { label: 'Herdr', tooltip: 'Executa o agente em pane isolado via modo nativo de agentes do Herdr.', recommended: true }, terminal: { label: 'Terminal direto', tooltip: 'Executa Codex, Claude ou Agy como processo local.' }, tmux: { label: 'Sessão tmux', tooltip: 'Executa o agente em uma sessão tmux legada.' } },
